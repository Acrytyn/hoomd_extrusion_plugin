...
#ifndef __EXTRUSION_PAIR_GPU_H__
#define __EXTRUSION_PAIR_GPU_H__
...
/*! Derived from ExtrusionPair, this class provides exactly the same interface for computing
   ...
   te \a evaluator calculates \f$V(\vec r,\vec e_i, \vec e_j)\f$ in a generic way.
    ...
    \sa export_ExtrusionPairGPU()
*/
template<class evaluator> class ExtrusionPairGPU : public ExtrusionPair<evaluator>
    {
    public:
    //! Construct the pair potential
    ExtrusionPairGPU(std::shared_ptr<SystemDefinition> sysdef,
                          std::shared_ptr<NeighborList> nlist);
    //! Destructor
    virtual ~ExtrusionPairGPU() { };
...
ExtrusionPairGPU<evaluator>::ExtrusionPairGPU(std::shared_ptr<SystemDefinition> sysdef,
                                                        std::shared_ptr<NeighborList> nlist)
    : ExtrusionPair<evaluator>(sysdef, nlist)
    {
    ...
    if (!this->m_exec_conf->isCUDAEnabled())
        ...
        this->m_exec_conf->msg->error()
            ...
            << ": Creating a ExtrusionPairGPU with no GPU in the execution configuration"
            << std::endl
            << std::endl;
        throw std::runtime_error("Error initializing ExtrusionPairGPU");
        }
...
template<class evaluator> void ExtrusionPairGPU<evaluator>::computeForces(uint64_t timestep)
    {
    ...
    if (third_law)
        ...
        this->m_exec_conf->msg->error()
            << "ai_pair." << evaluator::getName()
            << ": ExtrusionPairGPU cannot handle a half neighborlist" << std::endl
            << std::endl;
        throw std::runtime_error("Error computing forces in ExtrusionPairGPU");
        }
...
template<class evaluator>
void ExtrusionPairGPU<evaluator>::setParams(unsigned int typ1,
                                                 unsigned int typ2,
                                                 const typename evaluator::param_type& param)
    {
    ExtrusionPair<evaluator>::setParams(typ1, typ2, param);
    this->m_params[this->m_typpair_idx(typ1, typ2)].set_memory_hint();
    this->m_params[this->m_typpair_idx(typ2, typ1)].set_memory_hint();
    }

template<class evaluator>
void ExtrusionPairGPU<evaluator>::setShape(unsigned int typ,
                                                const typename evaluator::shape_type& shape_param)
    {
    ExtrusionPair<evaluator>::setShape(typ, shape_param);
...
/*! \param name Name of the class in the exported python module
    \tparam T Class type to export. \b Must be an instantiated ExtrusionPairGPU class template.
    \tparam Base Base class of \a T. \b Must be PotentialPair<evaluator> with the same evaluator as
   used in \a T.
*/
template<class T> void export_ExtrusionPairGPU(pybind11::module& m, const std::string& name)
    {
    pybind11::class_<ExtrusionPairGPU<T>,
                     ExtrusionPair<T>,
                     std::shared_ptr<ExtrusionPairGPU<T>>>(m, name.c_str())
...
#endif // ENABLE_HIP
#endif // __EXTRUSION_PAIR_GPU_H__

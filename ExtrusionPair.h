#ifndef __Extrusion_PAIR_H__
#define __Extrusion_PAIR_H__
...
/*! \file ExtrusionPair.h
    \brief Defines the template class for extrusion pair potentials
...
/*! <b>Overview:</b>
    ExtrusionPair computes standard pair potentials (and forces) between all particle pairs in
   ...
   not performed directly by this class, but by an extrusion_evaluator class (e.g. EvaluatorPairLJ)
   which is passed in as a template parameter so the computations are performed as efficiently as
   possible.

    ExtrusionPair handles most of the gory internal details common to all standard pair
   potentials.
   ...
   is defined by \a param_type in the potential extrusion_evaluator class passed in. See the appropriate
   documentation for the extrusion_evaluator for the definition of each element of the parameters.
*/

template<class extrusion_evaluator> class ExtrusionPair : public ForceCompute
    {
    public:
    //! Param type from extrusion evaluator
    typedef typename extrusion_evaluator::param_type param_type;

    //! Shape param type from extrusion evaluator
    typedef typename extrusion_evaluator::shape_type shape_type;

    //! Construct the pair potential
    ExtrusionPair(std::shared_ptr<SystemDefinition> sysdef,
                       std::shared_ptr<NeighborList> nlist);
    //! Destructor
    virtual ~ExtrusionPair();
    ...
    std::vector<std::string> getTypeShapeMapping(
        ...
        for (unsigned int i = 0; i < type_shape_mapping.size(); i++)
            {
            extrusion_evaluator evaluator(dr, q, q, rcut, m_params[m_typpair_idx(i, i)]);
            if (extrusion_evaluator::needsShape())
                {
...
*/
template<class extrusion_evaluator>
ExtrusionPair<extrusion_evaluator>::ExtrusionPair(std::shared_ptr<SystemDefinition> sysdef,
                                                        std::shared_ptr<NeighborList> nlist)
    : ForceCompute(sysdef), m_nlist(nlist), m_shift_mode(no_shift),
      m_typpair_idx(m_pdata->getNTypes())
    {
    m_exec_conf->msg->notice(5) << "Constructing ExtrusionPair<" << extrusion_evaluator::getName()
                                << ">" << std::endl;
    assert(m_pdata);
    assert(m_nlist);
...
template<class extrusion_evaluator> ExtrusionPair<extrusion_evaluator>::~ExtrusionPair()
    {
    m_exec_conf->msg->notice(5) << "Destroying ExtrusionPair<" << extrusion_evaluator::getName()
                                << ">" << std::endl;
...
*/
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setParams(unsigned int typ1,
                                                    unsigned int typ2,
...
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setParamsPython(pybind11::tuple typ,
...
template<class extrusion_evaluator>
pybind11::object ExtrusionPair<extrusion_evaluator>::getParamsPython(pybind11::tuple typ)
    {
    auto typ1 = m_pdata->getTypeByName(typ[0].cast<std::string>());
    auto typ2 = m_pdata->getTypeByName(typ[1].cast<std::string>());
...
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::validateTypes(unsigned int typ1,
                                                        unsigned int typ2,
                                                        std::string action)
    {
...
*/
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setShape(unsigned int typ, const shape_type& shape_param)
    {
    if (typ >= m_pdata->getNTypes())
        {
        throw std::runtime_error("Error setting shape parameters in ExtrusionPair");
        }
...
*/
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setShapePython(std::string typ,
...
*/
template<class extrusion_evaluator>
pybind11::object ExtrusionPair<extrusion_evaluator>::getShapePython(std::string typ)
    {
    auto typ_ = m_pdata->getTypeByName(typ);
    if (typ_ >= m_pdata->getNTypes())
        {
        throw std::runtime_error("Error getting shape parameters in ExtrusionPair");
        }
...
*/
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setRcut(unsigned int typ1, unsigned int typ2, Scalar rcut)
    {
...
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::setRCutPython(pybind11::tuple types, Scalar r_cut)
    {
...
template<class extrusion_evaluator>
Scalar ExtrusionPair<extrusion_evaluator>::getRCut(pybind11::tuple types)
    {
    auto typ1 = m_pdata->getTypeByName(types[0].cast<std::string>());
    auto typ2 = m_pdata->getTypeByName(types[1].cast<std::string>());
...
*/
template<class extrusion_evaluator>
void ExtrusionPair<extrusion_evaluator>::computeForces(uint64_t timestep)
    {
    ...
    ArrayHandle<Scalar> h_rcutsq(m_rcutsq, access_location::host, access_mode::read);
        ...
        for (int i = 0; i < (int)m_pdata->getN(); i++)
            ...
            if (extrusion_evaluator::needsCharge())
                qi = h_charge.data[i];
            ...
            for (unsigned int k = 0; k < size; k++)
                ...
                extrusion_evaluator eval(dx, quat_i, quat_j, rcutsq, param);

                if (extrusion_evaluator::needsCharge())
                    eval.setCharge(qi, qj);
                if (extrusion_evaluator::needsShape())
                    eval.setShape(&m_shape_params[typei], &m_shape_params[typej]);
                if (extrusion_evaluator::needsTags())
 ...
 */
template<class extrusion_evaluator>
CommFlags ExtrusionPair<extrusion_evaluator>::getRequestedCommFlags(uint64_t timestep)
    {
...
*/
template<class T> void export_ExtrusionPair(pybind11::module& m, const std::string& name)
    {
    pybind11::class_<ExtrusionPair<T>, ForceCompute, std::shared_ptr<ExtrusionPair<T>>>
        extrusionpair(m, name.c_str());
    extrusionpair
        .def(pybind11::init<std::shared_ptr<SystemDefinition>, std::shared_ptr<NeighborList>>())
        .def("setParams", &ExtrusionPair<T>::setParamsPython)
        .def("getParams", &ExtrusionPair<T>::getParamsPython)
        .def(("set" + T::getShapeParamName()).c_str(), &ExtrusionPair<T>::setShapePython)
        .def(("get" + T::getShapeParamName()).c_str(), &ExtrusionPair<T>::getShapePython)
        .def("setRCut", &ExtrusionPair<T>::setRCutPython)
        .def("getRCut", &ExtrusionPair<T>::getRCut)
        .def_property("mode",
                      &ExtrusionPair<T>::getShiftMode,
                      &ExtrusionPair<T>::setShiftModePython)
        .def("getTypeShapesPy", &ExtrusionPair<T>::getTypeShapesPy);
    }
